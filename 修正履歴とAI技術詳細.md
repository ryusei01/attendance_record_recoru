...
#### 7. URL 指定機能とプロファイルパス指定機能

**実装内容**:

- Recoru の勤怠入力ページ URL を指定可能に
- Chrome のプロファイルパスを指定可能に（ログイン状態の保持）

**主な機能**:

- `--url`オプション（コマンドライン）
- `--profile`オプション（コマンドライン）
- `config.json`での`base_url`設定
- `config.json`での`profile_path`設定
- Streamlit GUI での URL 入力フィールド
- Streamlit GUI でのプロファイルパス入力フィールド
...
- `login()`メソッドにリトライ機能を追加
- `_attempt_login()`メソッドで 1 回のログイン試行
- `config.json`での`login_retry_count`設定（デフォルト: 3 回）
- `config.json`での`login_retry_interval`設定（デフォルト: 5 秒）
...
- 正常完了時のブラウザ保持
- エラー時の自動クローズ
- ログイン失敗時のブラウザ保持（手動確認用）

#### 10. 入力処理の最適化

**実装内容**:

- 各レコード入力ごとのページリロードを廃止
- URL 確認による最適化
- 既存入力のスキップ機能

**主な機能**:

- `input_attendance()`メソッドに`skip_reload`パラメータを追加
- `input_multiple_attendance()`で最初の 1 回だけページを読み込み
- 既に出勤時刻や退勤時刻が入力されている場合はスキップ
- 日（day）を基準に行を検索する機能
...
- すべてのレコードに対して出勤区分を「1」（出勤）に固定

**主な機能**:

- レコードの状態に関係なく、常に「出勤」として入力
...
- 日付ベースの解析方式への変更
- 曜日が検出できない場合のフォールバック処理

**主な機能**:

- 日（1, 2, 3...）の順番でレコードを検出
- 曜日が見つからない場合でも、日付の順番からレコードを抽出
- 時刻パターン（HH.MM 形式）と日付の区別を改善
- 欠落している日付の空レコードも作成

**技術的詳細**:

- 日付マップを作成して、日付とトークンインデックスの対応を管理
- 次の日付が見つかるまでの範囲で時刻を検索
- 最初の 2 つの時刻を開始時刻と退勤時刻として使用
- エラーハンドリングの改善
- ユーザーフレンドリーなエラーメッセージ
...
### URL 指定機能とプロファイルパス指定機能

- Recoru の勤怠入力ページ URL を`--url`オプションまたは`config.json`で指定可能
- Chrome のプロファイルパスを`--profile`オプションまたは`config.json`で指定可能
- ログイン状態を保持するためにプロファイルを使用可能

### ログインリトライ機能

- ログイン失敗時の自動リトライ機能を実装
- リトライ回数と間隔を`config.json`で設定可能
- デフォルト: 3 回までリトライ、5 秒間隔

### ブラウザ管理の改善

- 正常完了時はブラウザを開いたままに（手動確認用）
- エラー発生時は自動的にブラウザを閉じる
- ログイン失敗時はブラウザを開いたままに（手動確認用）

### 入力処理の最適化

- 各レコード入力ごとのページリロードを廃止
- URL 確認による最適化（既に正しいページにいる場合はリロードしない）
- 既に入力がある場合は自動的にスキップ
- 日（day）を基準に行を検索する機能
...
- 日付ベースの解析方式に変更
- 曜日が検出できない場合でも、日（1, 2, 3...）の順番でレコードを抽出
- 時刻パターンと日付の区別を改善
